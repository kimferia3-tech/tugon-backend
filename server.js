...
const transporter = nodemailer.createTransport({
    ...
    auth: {
        ...
        pass: process.env.EMAIL_PASS || 'ueym uihi aduq frzp'
    }
});
...
// ADD NEW APPLICANT (to new_applications)
app.post('/applications', async (req, res) => {
    ...
    try {
        ...
        res.status(200).json({ message: "Applicant registered successfully!", application: result.rows[0] });
    } catch (err) {
...
app.post('/api/settings', async (req, res) => {
    ...
    try {
        const queryText = `
            UPDATE new_settings 
            SET standard_amount = $1, total_funds = $2, auto_approval = $3, email_notifications = $4 
            WHERE id = 1 
            RETURNING *;
        `;
        const values = [standardAmount || 3000.00, totalFunds || 100000.00, autoApproval || 'Disabled', emailNotifications || 'Enabled'];
        const result = await pool.query(queryText, values);
...
app.post('/api/admin/profile', async (req, res) => {
    ...
    try {
        ...
        res.status(200).json({ message: "Admin profile updated successfully!", profile: result.rows[0] });
    } catch (err) {
...
// =======================================================
// --- 8. NEW WEBINARS ENDPOINTS (new_webinars) ---
// =======================================================

app.get('/api/webinars', async (req, res) => {
    try {
        const result = await pool.query("SELECT *, TO_CHAR(schedule_date, 'Mon DD, YYYY HH:MI AM') as date_formatted FROM new_webinars ORDER BY schedule_date ASC");
        res.status(200).json(result.rows);
    } catch (err) {
...
app.post('/api/webinars', async (req, res) => {
    ...
    try {
        const queryText = `
            ...
            RETURNING *;
        `;
        const result = await pool.query(queryText, [title, description, schedule_date || new Date()]);
        res.status(200).json({ message: "Webinar created successfully!", webinar: result.rows[0] });
    } catch (err) {
